# MineGuardian — Master Project Plan
### Open-source AI Safety & Ventilation Co-Pilot for Underground Mines
...
5. [Our Solution — MineGuardian](#5-our-solution--mineguardian)
...
21. [Appendix F — Sources & Links](#appendix-f--sources--links)
...
**In one sentence:** MineGuardian is a free, open-source AI "co-pilot" that watches an underground mine's sensors, spots danger before a human would, explains it in plain English, recommends the exact action, and can even turn up the ventilation fan itself — all running locally on a laptop at **zero AI cost**.

**The 60-second pitch (say this to a judge):**
> "Deep mines are already full of sensors. Maestro Digital Mine has air-quality stations running in Vale's Sudbury mines — including Creighton, which is 2.4 km deep — since 2015. Sandvik's Newtrax platform monitors Glencore's people and equipment underground. Vale runs a remote operations centre right here in Sudbury watching all five Ontario mines. So the data exists. **What's missing is the brain and the face.** That raw data still lands as numbers on a screen that a tired supervisor has to interpret under pressure, and the smart interpretation layer is either absent or locked inside expensive proprietary boxes. MineGuardian is that missing layer, built with **100% free open-source AI**: it reads the sensors off this little microcontroller, detects the anomaly with machine learning, explains *why* it matters in plain English with a local language model, tells the operator what to do, and drives the fan automatically. This is a proof-of-concept on a $40 board — but the exact same software drops straight onto the infrastructure Vale and Glencore already run."
...
**Why it's credible coming from you:** You built the **LoopX Safe & Smart Mining Data Platform** (real-time underground safety dashboards for autonomous equipment), an **offline-first underground inspection app for Vale at Minax**, **Random Forest ML at Laurentian's Mineral Exploration Research Centre** (published in *Ore Geology Reviews*), and an **LLM assistant over operational data at Flosonics**. MineGuardian is the intersection of all four. You are arguably the single most qualified person in that room to build exactly this.
...
**The takeaway for the pitch:** The Sudbury mining-tech cluster is world-class at **collecting** dangerous-environment data and at **keeping humans out of danger physically**. Everyone in that room respects these companies. MineGuardian doesn't compete with them — it **completes** them with the one layer they under-invest in.
...
> **One-line framing for the judges:** "Vale and Glencore have the *eyes* (sensors) and increasingly the *muscles* (automation). MineGuardian is the open-source, zero-cost **brain and friendly face** that connects them — and we prove it end-to-end on a $40 board."
...
## 5. Our Solution — MineGuardian

**MineGuardian** is a proof-of-concept "digital safety co-pilot" for an underground mine zone. It has four jobs:
...
4. **Act** — A **Ventilation-on-Demand** controller decides the right fan speed from occupancy + air quality and sends the command **back to the Pico W**, which physically **spins the kit's motor/fan** up or down and shows status on its little LCD — a real, visible closed loop.
...
**Product codename:** *MineGuardian.* (Alternative names if you prefer: *DeepVent AI*, *NorthGuard*, *StopeSense*. Pick one and use it consistently on every slide and screen.)
...
| Judging factor (typical hackathon) | How MineGuardian scores |
| --- | --- |
...
**Your kit:** *SunFounder Raspberry Pi Pico W Ultimate Starter Kit (Kepler Kit)* — 450+ parts, fully plug-and-play on a breadboard, no soldering. Everything we need is **in the box**.
...
| Role in MineGuardian | Kit component | Interface | Suggested Pico W pin(s) |
| --- | --- | --- | --- |
...
├── data/
│   └── mineguardian.db        # SQLite (gitignored)
└── docs/
    ├── architecture.png
    └── demo-script.md
```
...
6. Write a 3-line `llm.py` that calls Ollama and prints a response — confirm Python → local LLM works end-to-end.

**🤖 Cursor prompt to paste**
> "Create a Python module `backend/llm.py` using the `ollama` Python package. Expose `generate_alert(context: dict) -> str` and `ask_copilot(question: str, recent_rows: list[dict]) -> str`. Both call a local model (default `llama3.2:3b`, configurable via env var `MG_MODEL`). Keep outputs under 2 sentences. Add a `__main__` block that runs a sample call and prints the result. Include a timeout and a graceful fallback string if Ollama isn't running."
...
>
> (LLM prompt design) "In `backend/llm.py`, set the alert system prompt to: 'You are MineGuardian, an underground-mine safety co-pilot. Given sensor JSON and detected reasons, write ONE urgent sentence a shift supervisor can act on: state the zone, the hazard, and the single recommended action. No preamble.' Provide `generate_alert(zone, reading, reasons)`. Add `ask_copilot(question, recent_rows)` that answers from the provided rows only, and says 'insufficient data' if unknown."
...
- [ ] The local LLM produces a correct, ≤ 2-sentence alert naming the zone, hazard, and action — in < 4 s, offline.
...
2. **Gauges & trends:** vibration, temp/humidity, CO (SIM tag), airflow — Recharts sparklines.
...
### PHASE 4 — Integration, Closed Loop & Demo Polish
...
4. **Rehearse the demo script (§12) three times**, including the failure-recovery line ("and because it's all local, even if I lose the cable, the system keeps running in simulation — here").
...
> "CO is climbing in Zone 3, and there are two workers there. A human watching forty numbers might miss this. MineGuardian didn't."
...
> "And it doesn't just warn — it acts. Ventilation-on-Demand just raised the fan, on the screen **and on the board**. This is exactly the logic Vale uses underground to save millions — but here it's open-source."
...
**[2:20 — Reset + second scenario]** *Reset to green; trigger 'Bearing Failure'.*
...
> **Failure-recovery line (keep in your back pocket):** "Because it's all local, if I lose this cable the system keeps running in simulation — *like this* —" *switch to `make sim`* "— so it never goes dark."
...
1. **Title + promise.** "MineGuardian — open-source AI safety & ventilation co-pilot for underground mines." Dashboard screenshot. Your name + "$0 AI cost."
...
| --- | --- | --- | --- | --- |
| R1 | Kit arrives late / DOA | Low | High | Build & demo entirely in **`make sim`** mode — full pipeline needs no hardware. Hardware is the garnish, not the meal. |
...
> "Write a `README.md` for MineGuardian: one-line value prop, the problem (name Vale/Maestro/Glencore/Newtrax), an architecture image, the open-source/$0 AI stack, and a Quick Start (`make demo` and hardware-free `make sim`). Tone: confident, concrete, recruiter-readable."
...
| **scikit-learn IsolationForest** | — | tiny | Instant | Anomaly detection | ✅ **Required** |
| `faster-whisper` (base) | — | ~0.2 GB | Near real-time | Voice input | ◻ Stretch |

**Operating rules:** keep prompts short; pre-warm before demo; set `MG_MODEL` env var to switch models without code changes; if a model stalls, fall back to `gemma2:2b`.
...
| Dataset | What it is | Use in MineGuardian | Where |
| --- | --- | --- | --- |
